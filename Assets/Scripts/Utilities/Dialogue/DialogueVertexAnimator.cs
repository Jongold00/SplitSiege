...
public class DialogueVertexAnimator
{
    ...
    public IEnumerator AnimateTextIn(List<DialogueUtility.DialogueCommand> commands, string processedMessage, Action onFinish)
    {
        textAnimating = true;
    ...
    private void ExecuteCommandsForCurrentIndex(List<DialogueUtility.DialogueCommand> commands, int visableCharacterIndex, ref float secondsPerCharacter, ref float timeOfLastCharacter)
    {
        for (int i = 0; i < commands.Count; i++)
        {
            DialogueUtility.DialogueCommand command = commands[i];
            if (command.position == visableCharacterIndex)
            {
                switch (command.type)
                {
                    case DialogueUtility.DialogueCommandType.Pause:
                        timeOfLastCharacter = Time.unscaledTime + command.floatValue;
                        break;
                    case DialogueUtility.DialogueCommandType.TextSpeedChange:
    ...
    private Vector3 GetAnimPosAdjustment(TextAnimInfo[] textAnimInfo, int charIndex, float fontSize, float time)
    {
        ...
        for (int i = 0; i < textAnimInfo.Length; i++)
        {
            ...
            {
                if (info.type == DialogueUtility.TextAnimationType.shake)
                {
                ...
                else if (info.type == DialogueUtility.TextAnimationType.wave)
                {
    ...
    private TextAnimInfo[] SeparateOutTextAnimInfo(List<DialogueUtility.DialogueCommand> commands)
    {
        List<TextAnimInfo> tempResult = new List<TextAnimInfo>();
        List<DialogueUtility.DialogueCommand> animStartCommands = new List<DialogueUtility.DialogueCommand>();
        List<DialogueUtility.DialogueCommand> animEndCommands = new List<DialogueUtility.DialogueCommand>();
        for (int i = 0; i < commands.Count; i++)
        {
            DialogueUtility.DialogueCommand command = commands[i];
            if (command.type == DialogueUtility.DialogueCommandType.AnimStart)
            {
                animStartCommands.Add(command);
                commands.RemoveAt(i);
                i--;
            }
            else if (command.type == DialogueUtility.DialogueCommandType.AnimEnd)
            {
        ...
        else
        {
            ...
            {
                DialogueUtility.DialogueCommand startCommand = animStartCommands[i];
                DialogueUtility.DialogueCommand endCommand = animEndCommands[i];
...
public struct TextAnimInfo
{
    ...
    public int endIndex;
    public DialogueUtility.TextAnimationType type;
}